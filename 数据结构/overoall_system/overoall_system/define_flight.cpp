		...
		case 2:
		{
			...
			while (1)
			{
				if (flag2)
				{
					...
					{
						if (time_end.get_year()<time_begin.get_year() || time_end.get_year() >(time_begin.get_year() + 1) || ((time_end.get_year() == time_begin.get_year() + 1) && (time_end.get_month() != 1 || time_end.get_day() > 2)) || (time_end.get_year() == time_begin.get_year() && time_end.get_month() == time_begin.get_month() && time_end.get_day() < time_begin.get_day()))
						{
...
#pragma warning(suppress : 4996)//关闭警告
	...
	bool flag = true;
	if (year<(p.tm_year + 1900) || year>(p.tm_year + 1901) || (year==p.tm_year&&month<p.tm_mon) ||(year==p.tm_year+1901&&month>2)||month<0|| month>(p.tm_mon + 3) || month > 12 || hours <= 0 || hours > 24 || minite <= 0 || minite > 60 || day < 0||((year == (p.tm_year+1900) && month == (1+p.tm_mon))&&day<p.tm_mday))
	{
		flag = false;
	}
	if (flag)
	{
		if (!is_day_legal(year, month, day))
			flag = false;
	}
	return flag;
}
